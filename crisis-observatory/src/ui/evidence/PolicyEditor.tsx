...
export default function PolicyEditor({ original, onClose }: { original: Policy | null; onClose: () => void }) {
  ...
  const [draft, setDraft] = useState<Policy>(original ?? { id: "", title: "", body: "", kind: "custom", severity: "warning" });
  const [error, setError] = useState("");
  const [removing, setRemoving] = useState(false);
  useEffect(() => { dialog.current?.showModal(); }, []);
  function save(deleted = false) {
    try { savePolicy({ ...draft, deleted }, original ?? undefined); onClose(); }
    catch(e) { setError(e instanceof Error ? e.message : "No se pudo guardar en este navegador."); }
  }
  return <dialog ref={dialog} className="policy-editor" onCancel={onClose} aria-labelledby="policy-editor-title">
    <form onSubmit={e => { e.preventDefault(); save(); }}>
      ...
      <label>Situación que requiere escalado<select value={draft.kind} onChange={e=>setDraft({...draft,kind:e.target.value as Policy["kind"]})}>{Object.entries(escalationKinds).map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label>
      ...
      <p className="policy-search-help">Respuesta prevista: solicitar una decisión humana mediante la alerta a pantalla completa. Guardado como borrador local; pendiente de conexión con el agente.</p>
      ...
      <footer>{original && !original.deleted && <button type="button" className="policy-danger" onClick={()=>setRemoving(true)}>Eliminar política</button>}<button type="button" onClick={onClose}>Cancelar</button><button className="policy-primary" type="submit">{original?.deleted ? "Restaurar política" : "Guardar política"}</button></footer>
    </form>
  </dialog>;
}
