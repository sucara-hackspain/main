...
const rows = moments.flatMap((m) => {
  const [g, b, p] = arms.map(([, policy]) => deaths(policy, m.id));
  return g.length && b.length && p.length ? [{ id: m.id, family: m.family, why: m.handover?.why ?? "", greedy: mean(g), basic: mean(b), plan: mean(p) }] : [];
});
