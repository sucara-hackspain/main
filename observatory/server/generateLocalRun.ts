...
import { DanaMaster, EscalationDesk, Graph, GreedyCoordinator, makeTickRecord, Simulation, type GraphData, type RunMeta } from "../src/engine";
import { readEscalation } from "../src/escalationFile";
...
const graph = new Graph(JSON.parse(readFileSync(resolve(root, "data/valencia.json"), "utf8")) as GraphData);
const sim = new Simulation({ graph, seed, master: new DanaMaster(), coordinator: new GreedyCoordinator() });
const dir = resolve(root, "runs", id);
mkdirSync(dir, { recursive: true });
// The same escalation catalogue the engine applies in a traced run, read from policies/.
const escalation = readEscalation(resolve(root, "policies/escalation.json"));
