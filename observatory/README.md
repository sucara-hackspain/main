...
> **Formato soportado.** Control Center lee el modelo de unidades e incidentes del motor de `src/engine/` (`frame.units`, `scenes`, `incidents`, `knownWater`, `knownClosedEdges`; eventos `call_*`, `scene_*`, `victim_*`, `unit_*`, `flood_*`). `src/ui/engineTrace.ts` reexporta sus tipos y reúne las etiquetas y los helpers compartidos. Las grabaciones del contrato anterior (`frame.ambulances`, `patients`) se rechazan con un mensaje explícito.
...
| `server/` | API de lectura que sirve las ejecuciones, el callejero y el catálogo de escalado mediante Vite. |
| `src/engine/`, `src/coordinators/`, `src/masters/`, `src/phone/`, `src/triage/` | El motor y sus agentes, en este mismo paquete. Ver [`../docs/engine.md`](../docs/engine.md). |
| `src/memory/`, `src/lab/` | La doctrina del agente y el laboratorio que la entrena. |
| `data/`, `lab/`, `policies/`, `memory/`, `runs/` | Callejero, noches congeladas, catálogo de escalado, memoria y grabaciones. |
| `viewer/` | Visor del motor (verdad contra creencia, grafo de memoria), aparte de esta UI. |

## Arranque

Desde este directorio (`observatory/`):
...
La aplicación lee las ejecuciones de `runs/`. Si todavía no hay ninguna, muestra el estado de espera. Las nuevas ejecuciones aparecen automáticamente en el selector; la selección se conserva mientras se revisa una ejecución.

`npm run data:local` ejecuta el motor con el coordinador por reglas (semilla 2, 120 registros) y escribe la ejecución en `runs/`. El generador de pruebas local usa únicamente el motor, sin requerir el SDK de HappyRobot ni inicializar la memoria persistente del agente. Las grabaciones del formato anterior que queden en esa carpeta muestran el error de formato; el selector sigue disponible para elegir otra.
...
Al seleccionar un ticket, el lateral muestra sus datos, unidades, información pendiente y cronología: llamadas, órdenes aceptadas o rechazadas, motivos registrados por el coordinador, valoraciones, cambios de prioridad y cierre. Los siguientes pasos deducidos del estado se identifican como tales; si falta una justificación se indica expresamente. Los tickets cerrados se conservan en el historial, sin usar información posterior al instante seleccionado. **Enfocar en el mapa** abre **Territorio** con la incidencia como protagonista: encuadra la incidencia y lo que interviene en ella (sus unidades con la ruta que les queda, el hospital de destino y, en supervisión, la escena real), atenúa el resto de marcadores y las áreas de las demás incidencias, y fija su detalle en una esquina para no tapar el encuadre. Cerrar el detalle devuelve el mapa completo. Si la incidencia ya salió del registro actual, abre el último instante en que aparecía.
...
| Panel de situación | `situation/SituationSidebar.tsx`, `situation/EntityCard.tsx`, `situation/entityParts.ts`, `situation/model.ts`, `situation/situation.css` | Resumen global y balance, agua y cortes, incidentes, unidades y hospitales (plegables, con altura máxima), filtros, y el detalle de cada entidad, que se abre en un modal sobre el mapa. |
...
`ControlCenter.tsx` conecta estas vistas mediante props y mantiene la ejecución, reproducción, filtros y selección compartida. El mapa y el panel de situación comparten una selección `{ kind, id }` (`incident`, `unit`, `hospital` o `scene`, definida en `engineTrace.ts`); el mapa y el panel comparten además el snapshot operativo y los conjuntos de entidades relacionadas y coincidentes con el filtro. `onSelect` actualiza la selección; `focusRequest` permite volver a centrarla. El detalle de la selección se abre en un modal sobre el mapa, junto a lo seleccionado, tanto si se elige en el mapa como en el panel; desde una incidencia, «Abrir ticket» lleva a su ticket. Los tickets mantienen su búsqueda, filtro y selección al cambiar de vista; el botón «Enfocar en el mapa» pone la incidencia seleccionada en primer plano (encuadre de lo que interviene, resto atenuado). Cada componente gestiona sus referencias al DOM y su scroll.
...
Los cambios del Control Center viven en `src/ui/`. El visor de `viewer/` es una aplicación independiente sobre el mismo motor.
...
Las peticiones las decide el motor, no la interfaz: su escritorio de escalado (`src/engine/escalation.ts`) aplica el catálogo de `policies/escalation.json` al cerrar cada registro y escribe en él lo que abre y lo que se cierra solo. `useInterventions.ts` las lee con `escalationsFrom()`; `detectInterventions()` se conserva para las grabaciones anteriores al escritorio, y la tabla de arriba describe el catálogo que trae el motor. Cada alerta cita la política que la pidió y enlaza con ella; cada ejecución conserva en `meta.json` el catálogo con el que se grabó. La página **Políticas de escalado** (`/escalation-policies`) edita ese mismo archivo a través de `GET`/`PUT /api/policies`: cambiar un umbral, desactivar una política o añadir otra cambia lo que escala el motor en la siguiente ejecución. Lo que se muestra en un registro no incluye lo que cuentan los posteriores; solo «Siguiente excepción» los recorre, para saltar hasta ella. Las recomendaciones y los ETA usan el grafo y la búsqueda de rutas de `../gabriel/src/engine/`, en modo lectura, con lo que sabe el coordinador en ese registro (cortes comunicados y agua conocida) y lo que puede hacer cada tipo de unidad: el rescate acuático cruza las calles inundadas conocidas y el helicóptero vuela en línea recta.
...
npm run test:engine     # tests del motor
...
Los tests de navegador combinan una noche DANA simulada en memoria con el motor de `src/engine/` y el coordinador por reglas (`tests/support/engineRun.ts`, semilla 2) con escenarios fijos para tickets e intervenciones (`tests/support/ticketRun.ts` y `tests/support/interventionRun.ts`). Todos se sirven a través del contrato incremental de la API, sin escribir en `runs/`. Comprueban snapshots, GPS, historial, filtros, navegación móvil e intervenciones del operador. Los escenarios fijos mantienen las pruebas de UI independientes de la estrategia del simulador. También verifican el rechazo explícito del formato anterior. Las teselas externas se sustituyen por un fondo local. Las capturas y resultados de pruebas se ignoran en Git.
