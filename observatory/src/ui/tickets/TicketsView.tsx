...
import { ticketNextStep, ticketStates, triageOf, type Ticket, type TicketState, type TicketStep } from "./model";
import "./tickets.css";
...
export function TicketDetail({ ticket, seconds, tick, onLocate, onClose, runs, runId, onRun, records }: {
  ...
  const triage = incident ? triageOf(incident) : null;
    ...
    {ticket && incident && next ? <div className="ticket-detail-body" ref={body}>
      <section className="ticket-summary">
        ...
        <dl className="ticket-facts"><div><dt>Prioridad actual</dt><dd><span className="ticket-priority" data-priority={incident.priority}><i />P{incident.priority} · {priority[incident.priority].label}</span>
            {triage && <small className="ticket-triage-read">{incident.located ? "El triaje del 112 la puso en" : "Suelo del triaje del 112:"} P{triage.priority} · {triage.reasoning}{incident.located ? " · manda la dotación en el lugar" : ""}</small>}</dd></div>
