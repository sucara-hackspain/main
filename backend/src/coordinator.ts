...
/**
 * Heurística: heridos por urgencia (ttl ascendente); a cada uno, la ambulancia libre más cercana.
...
export const greedyCoordinator: Coordinator = (w, map) => {
  ...
  let pending = w.injured.filter((h) => !alreadyTargeted.has(h.id)).sort((x, y) => x.ttl - y.ttl);
  ...
  for (const onlyIfInTime of [true, false]) {
    for (const h of [...pending]) {
      ...
      logEvent(w, `${a.id} → ${h.id} (eta ${turns(a, h)} turnos, ttl ${h.ttl}${onlyIfInTime ? "" : ", NO llega a tiempo"})`);
    }
  }
};
